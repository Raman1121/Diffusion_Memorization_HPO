...
class SVDConv2d(nn.Conv2d):
    ...
    def forward(self, x: torch.Tensor):
        ...
        weight_updated = (
            ...
            @ torch.diag(
                F.relu(self.S.to(x.device, dtype=x.dtype) + self.scale * self.delta)
            )
...
class SVDLinear(nn.Linear):
    def __init__(
        self, in_features: int, out_features: int, scale: float = 1.0, **kwargs
    ):
...
class SVDEmbedding(nn.Embedding):
    ...
    def __init__(
        self, num_embeddings: int, embedding_dim: int, scale: float = 1.0, **kwargs
    ):
    ...
    def forward(self, x: torch.Tensor):
        ...
        weight_updated = (
            self.U.to(x.device)
            @ torch.diag(F.relu(self.S.to(x.device) + self.scale * self.delta))
            @ self.Vh.to(x.device)
        )
        return F.embedding(
            x,
...
class SVDGroupNorm(nn.GroupNorm):
    def __init__(
        self, num_groups: int, num_channels: int, scale: float = 1.0, **kwargs
    ):
